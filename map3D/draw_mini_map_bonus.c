...
/*   Updated: 2025/08/24 12:52:38 by mradouan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
    ...
    while (i <= data->mini_m->end_y)
    {
        ...
        while (j <= data->mini_m->end_x)
        {
            ...
            if (i < 0 || j < 0 || i >= data->h_map || j >= data->arr_width[i])
                draw_square(data, data->mini_m->draw_x, data->mini_m->draw_y, 0x608a86);
            else if (data->map[i][j] == '1')
                draw_square(data, data->mini_m->draw_x, data->mini_m->draw_y, 0x3289a8);
            else if (data->map[i][j] == 'D')
                draw_square(data, data->mini_m->draw_x, data->mini_m->draw_y, 0x805505);
			else if (data->map[i][j] == 'O')
				draw_square(data, data->mini_m->draw_x, data->mini_m->draw_y, 0x15ad1c);
            else
                draw_square(data, data->mini_m->draw_x, data->mini_m->draw_y, 0x555555);
			j++;
        }
		i++;
    }
...
void draw_mini_map(t_data *data)
{
    ...
    draw_player(data, data->mini_m->center_x - 2, data->mini_m->center_y - 2, 0xfcfcfc);
}

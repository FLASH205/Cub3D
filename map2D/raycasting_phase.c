...
static void	get_horizontal(t_data *data, float angle, t_player *player)
{
	...
	if (player->ray_face_left && x_step > 0)
		x_step *= -1;
	// if (player->ray_face_right && x_step < 0)
	// 	x_step *= -1;
	while (1)
	{
...
static void	get_vertical(t_data *data, float angle, t_player *player)
{
	...
	if (player->ray_face_up && y_step > 0)
		y_step *= -1;
	// if (player->ray_face_down && y_step < 0)
	// 	y_step *= -1;
	while (1)
	{
...
void	raycasting_phase(t_data *data, t_player *player, float angle)
{
	...
	while (i < WIDTH)
	{
		...
		player->ray_face_down = (ray_angle >= 0 && ray_angle < M_PI);
		player->ray_face_up = !player->ray_face_down;
		player->ray_face_left = (ray_angle > M_PI / 2 && ray_angle < 3 * M_PI / 2);

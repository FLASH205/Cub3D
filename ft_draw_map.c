...
/*   Updated: 2025/07/26 18:28:58 by ybahmaz          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_strip(float wall_strip, t_image *image, int x)
{
	int		i;
	float	center;
	...
	while (i < HEIGHT)
	{
		if (i <= HEIGHT / 2 - center)
			ft_put_pixel(image, 0x0069DE, x, i);
		else if (i >= HEIGHT / 2 + center)
		...
		// 	ft_put_pixel(image, 0x000000, x, i);
			...
			ft_put_pixel(image, 0xffea8c, x, i);
		i++;
	}
...
void	ft_draw_map(t_data *data)
{
	...
	while (i < WIDTH)
	{
		...
		draw_strip(wall_strip, data->image, i);
		i++;
	}
}
